...
if [ ${PLUGINS_EXIT_CODE} -eq 0 ] && [ -f "${PLUGINS_RESPONSE_FILE}" ]; then
    ...
    LIVE_SITE_AUTH_TOKEN="$LIVE_SITE_AUTH_TOKEN" LIVE_SITE_URL="$LIVE_SITE_URL" BACKUP_DIR="${BACKUP_DIR}" PLUGIN_SLUG="$PLUGIN_SLUG" node scripts/process-plugins.js < "${PLUGINS_RESPONSE_FILE}"
    PROCESS_EXIT_CODE=$?
...
if [ ${THEMES_EXIT_CODE} -eq 0 ] && [ -f "${THEMES_RESPONSE_FILE}" ]; then
    ...
    LIVE_SITE_AUTH_TOKEN="$LIVE_SITE_AUTH_TOKEN" LIVE_SITE_URL="$LIVE_SITE_URL" BACKUP_DIR="${BACKUP_DIR}" THEME_SLUG="$THEME_SLUG" node scripts/process-themes.js < "${THEMES_RESPONSE_FILE}"
    PROCESS_EXIT_CODE=$?
