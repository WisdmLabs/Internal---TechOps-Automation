    ...
    add_action( 'wp_head', function() {
        ...
        if ( is_single() ) {
            ...
            if ( !empty($post_id) ) {
                ...
                if ( !empty( $tdb_template_id ) ) {
                    // load the cloud template
                    $post_template = new WP_Query(array(
                            'p' => $tdb_template_id,
                            'post_type' => 'tdb_templates',
                        )
                    );
                    // get shortcode atts
                    $tdb_featured_img_atts = tdb_util::get_shortcode_atts( $post_template->post->post_content, 'tdb_single_featured_image' );
                    $tdb_bg_featured_img_atts = tdb_util::get_shortcode_atts( $post_template->post->post_content, 'tdb_single_bg_featured_image' );
