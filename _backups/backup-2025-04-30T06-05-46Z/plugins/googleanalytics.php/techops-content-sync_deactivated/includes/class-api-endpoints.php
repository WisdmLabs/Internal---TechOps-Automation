...
class API_Endpoints {
    ...
    public function __construct() {
        $this->git_handler = new Git_Handler();
        $this->installer = new Installer();
        $this->rate_limiter = new Rate_Limiter(10, 60); // 10 requests per minute
    }
    ...
    public function register_routes() {
        ...
        register_rest_route('techops/v1', '/git/download', [
            ...
            'args' => [
                'repo_url' => [
                    ...
                    'validate_callback' => [$this, 'validate_git_url']
                ],
                'folder_path' => [
                    'required' => true,
                    'type' => 'string',
        ...
        // Add endpoint to get repository branches and tags
        register_rest_route('techops/v1', '/git/branches-tags', [
            'methods' => 'POST',
            'callback' => [$this, 'get_repo_branches_tags'],
            'permission_callback' => [$this, 'check_permission'],
            'args' => [
                'repo_url' => [
                    'required' => true,
     ...
     */
    public function validate_git_url($url) {
        $allowed_domains = [
            'github.com',
            'gitlab.com',
            'bitbucket.org'
    ...
    /**
     * Handle Git repository download and installation
     *
    ...
    public function handle_git_download($request) {
        try {
            ...
            $branch_or_tag = $request->get_param('branch_or_tag');
            $type = $request->get_param('type');

            // Download the folder as zip
            $result = $this->git_handler->download_folder_as_zip($repo_url, $folder_path, $branch_or_tag);
            if (is_wp_error($result)) {
                return $result;
            }

            // Install the package
            $install_result = $this->installer->install_from_zip($result['zip_path'], $type);
            if (is_wp_error($install_result)) {
                return $install_result;
            }

            // If it's a plugin, activate it
            if ($type === 'plugin' && isset($install_result['slug'])) {
                $activation_result = $this->process_plugin_activation($install_result['slug']);
            ...
            return new \WP_REST_Response([
                'success' => true,
                'message' => sprintf('%s installed and activated successfully', ucfirst($type)),
                'data' => $install_result
            ], 200);
     ...
     * @return WP_REST_Response|WP_Error
     */
    public function get_repo_branches_tags($request) {
        try {
            $repo_url = $request->get_param('repo_url');
            
            $result = $this->git_handler->get_branches_and_tags($repo_url);
            if (is_wp_error($result)) {
                return $result;
            }
