...
function techops_content_sync_admin_page() {
    ...
    <div class="wrap">
        ...
        <form method="post" action="">
            <?php wp_nonce_field('techops_plugin_update', 'techops_plugin_update_nonce'); ?>
            <table class="form-table">
                <tr>
                    <th scope="row">Update Status</th>
                    <td>
                        <?php
                        $log_file = WP_CONTENT_DIR . '/techops-update-log.txt';
                        if (file_exists($log_file)) {
                ...
                <tr>
                    <th scope="row">Last Update Results</th>
                    <td>
                        <?php
                        $last_update = get_option('techops_last_update_results', array());
                        if (!empty($last_update)) {
                            echo '<div class="update-results">';
                            
                            // Processed updates
                            if (!empty($last_update['processed'])) {
                                echo '<h3>Successfully Updated</h3>';
                                echo '<ul class="processed-list">';
                                foreach ($last_update['processed'] as $item) {
                                    echo '<li>';
                                    echo esc_html(ucfirst($item['type'])) . ': ' . esc_html($item['slug']);
                                    echo ' (from ' . esc_html($item['from']) . ' to ' . esc_html($item['to']) . ')';
                                    echo '</li>';
                                }
                                echo '</ul>';
                            }
                            ...
                            // Failed updates
                            if (!empty($last_update['failed'])) {
                                echo '<h3>Failed Updates</h3>';
                                echo '<ul class="failed-list">';
                                foreach ($last_update['failed'] as $item) {
                                    echo '<li>';
                                    echo esc_html(ucfirst($item['type'])) . ': ' . esc_html($item['slug']);
                            ...
                            // Skipped updates
                            if (!empty($last_update['skipped'])) {
                                echo '<h3>Skipped Updates</h3>';
                                echo '<ul class="skipped-list">';
                                foreach ($last_update['skipped'] as $item) {
                                    echo '<li>';
                                    echo esc_html(ucfirst($item['type'])) . ': ' . esc_html($item['slug']);
                                    if (isset($item['reason'])) {
                                        echo ' - ' . esc_html($item['reason']);
                            ...
                            echo 'No update results available.';
                        }
                        ?>
                    </td>
                </tr>
            ...
            <p class="submit">
                <input type="submit" name="techops_force_update" class="button button-primary" value="Force Update Plugins">
            </p>
        </form>
    </div>
    <?php
...
// Handle plugin update requests
function techops_content_sync_handle_update() {
    if (!current_user_can('manage_options')) {
        return;
    }

    if (isset($_POST['techops_force_update']) && check_admin_referer('techops_plugin_update', 'techops_plugin_update_nonce')) {
        $github_handler = new TechOpsContentSync\GitHub_Handler();
        $result = $github_handler->download_file();

        if ($result['success']) {
            // Save update results
            update_option('techops_last_update_results', $result['update_result']['results']);
            
            add_settings_error(
                'techops_content_sync_messages',
                'techops_content_sync_message',
                'Plugin update process completed. Check the update log for details.',
                'updated'
            );
        } else {
            add_settings_error(
                'techops_content_sync_messages',
                'techops_content_sync_message',
                'Plugin update failed: ' . $result['message'],
                'error'
            );
        }
    }
}
add_action('admin_init', 'techops_content_sync_handle_update'); 